...
```{r}
model_opt = glm("dropout ~ 1 + exa_cfu_pass + exa_grade_average + highschool_grade", data = train, family=binomial)
summary(model_opt)
...
Effettuiamo una cross validation per valutare la bontà del modello.

I modelli: 
```{r}
library(boot)
model1 = glm("dropout ~ 1 + exa_cfu_pass + exa_grade_average + highschool_grade", data = cat_no_lev_df, family = binomial)
model2 = glm("dropout ~ 1 + exa_cfu_pass + exa_grade_average", data = cat_no_lev_df, family = binomial)
model3 = glm("dropout ~ 1 + exa_cfu_pass + exa_grade_average + highschool_grade + exa_cfu_pass*exa_grade_average", data = cat_no_lev_df, family = binomial)
model4 = glm("dropout ~ 1 + exa_cfu_pass + exa_grade_average + previousStudies + highschool_grade", data=cat_no_lev_df, family=binomial)

delta1 = cv.glm(data=cat_no_lev_df, glmfit=model1,K=10)$delta[1]
delta2 = cv.glm(cat_no_lev_df, model2,K=10)$delta[1]
delta3 = cv.glm(cat_no_lev_df, model3,K=10)$delta[1]
delta4 = cv.glm(cat_no_lev_df, model4,K=10)$delta[1]

y = c(delta1, delta2, delta3, delta4)

plot(1:4, y, type = "b", xlab = "Model", ylab = "CV delta", main = "Cross Validation", ylim = c(0.05,0.07),xaxt = "n")
...
model = glm("dropout ~ 1 + exa_cfu_pass + exa_grade_average", data = cat_no_lev_df, family = binomial)

# Define the UI
ui <- fluidPage(
  sliderInput("cfu", "CFU", min = 0, max = 40, value = 27),
  sliderInput("voti", "Voti", min = 0, max = 30, value = 18),
...
server <- function(input, output) {
    create_dataframe <- function(cfu, voti) {
    # Create a dataframe
    df <- data.frame(CFU = cfu, Voti = voti)
    
    # Return the dataframe
    return(df)
    }
    create_dataframe(input$cfu, input$voti)
